...
\begin{center}
    ...
    \end{center}
    Notons $r_0$ et $u_0$ la répartition initiale de la population en proportion. Notons $r_k$ la proportion de la population totale qui habite en zone rurale à la fin de la $k$-ième année et $u_k$ la proportion de population qui habite en zone urbaine pour la même année. S’agissant de proportion de population, on a, pour toute année $k$,
$$r_k + u_k = 1$$.
\begin{enumerate}
...
\begin{solution}
$E(t)=E_0(1+0,03)^t= E_0\, e^{\log (1,03)t}= 5 \times 10^3 e^{\log (1,03)t}$.
\end{solution}
...
\begin{solution}
    La formule de la question précédente donne $E(113) \simeq 1.4\times 10^5$ Twh.
\end{solution}
...
\begin{solution}
     ...
     $$
     = \frac{1}{\log (1,03)}.E_0 [ e^{\log (1,03)t}]_{0}^{T}= \frac{1}{\log (1,03)}.E_0.(e^{\log (1,03)T}-1)
     $$
\end{solution}
...
\begin{solution}
    Il faut d'abord calculer $E_T=m_T. c^2= 5\times 10^{35}$ J $\sim 10^{26}$ Twh. Il faut maintenant résoudre l'équation
     $$
     \frac{1}{\log (1,03)}*(5000).((1,03)^T-1)=10^{26}
     $$
     
     $$
     (1.03)^T=1+10^{26}*\log(1.03)\frac{1}{5000}
     $$
     
     $$T=1618$$
     
     \noindent donc $1618-120=1498$ ans puisque on à commencé en 1900.
...
\begin{solution}
    Il faut d'abord calculer $E_{soleil}=m_{soleil}. c^2= 2\times 10^{30}\times (3\times 10^8)^2 =2\times 10^{30}\times 9\times 10^{16}= 1.8\times 10^{47}$ J $\sim 5\times 10^{31}$ Twh. Il faut maintenant résoudre l'équation
     $$
     \frac{1}{\log (1,03)}*(5000).((1,03)^T-1)=5*10^{31}
     $$
     
     On trouve 
     $$T=2062$$
     
     Donc, $2062-120= 1942$ ans.
\end{solution}
...
\begin{solution}
 Même argument, calculer $E_{VL}=m_{VL}. c^2= 1.5\times 2\times 10^{42}\times (3\times 10^8)^2 =3\times 10^{42}\times 9\times 10^{16}= 2.7\times 10^{58}$ J $\sim 7.5\times 10^{41}$ Twh. Il faut maintenant résoudre l'équation
     $$
     \frac{1}{\log (1,03)}*(5000).((1,03)^T-1)=7.5\times 10^{41}
     $$
     
     On trouve 
     $$T=2854\,\,\, $$
     
     donc $2854-120=2734$ ans
\end{solution}
\end{enumerate}
\end{exo}
...
\begin{exo}[Bilan énergétique de la planète et effet de Serre, \href{http://step.ipgp.fr/images/e/ec/Bernard_geodynext_td1_corr.pdf}{Source Ici}]
...
Calculer la valeur de $\epsilon$ correspondant à la température réel de $285$ K.
...
\item L'observation de base pour les modèles de prévision pour le réchauffement climatique c'est le fait que le facteur $\epsilon$, que quantifie l'effet de Serre, est, en première approximation, proportionnel au logarithme de la concentration en $CO_2$ dans l'atmosphère (voir \href{https://agupubs.onlinelibrary.wiley.com/doi/pdf/10.1029/98GL01908}{ici})

$$
\epsilon= k\log([CO_2])
$$

Calculer $T$ en fonction de $[CO_2]$ calculer sa dérivée. Déterminer le tableau de variation de la fonction 
